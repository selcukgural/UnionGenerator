...
public class OneOfConverterGenerator : IIncrementalGenerator
{
    private static readonly DiagnosticDescriptor MissingFactoryDescriptor = new(
        id: "UG2001",
        title: "Generated union type missing expected factory method",
        messageFormat: "Union type '{0}' must expose a public static factory method '{1}({2})', for example: public static {3} {1}({2} value) => new /*Case*/({2} value)",
        category: "Usage",
    ...
    private static readonly DiagnosticDescriptor FactoryParameterMismatchDescriptor = new(
        id: "UG2002",
        title: "Factory method parameter type mismatch",
        messageFormat: "Factory method '{0}' on '{1}' has parameter type '{2}' but expected '{3}' - expected signature: public static {1} {0}({3} value)",
        category: "Usage",
        defaultSeverity: DiagnosticSeverity.Warning,
        isEnabledByDefault: true,
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        {
            var src = "namespace UnionGenerator.OneOfSourceGen { public static class GeneratedOneOfHelpers { public static string Info() => \"OneOf POC\"; } }";
            ctx.AddSource("GeneratedOneOfHelpers.g.cs", SourceText.From(src, Encoding.UTF8));
        });

        // Find class declarations that use [GenerateUnion] attribute
        ...
        context.RegisterSourceOutput(candidates, (spc, typeSymbol) =>
        {
            ...
            var candidateFactories = typeSymbol.GetMembers().OfType<IMethodSymbol>().Where(m => m.IsStatic && m.DeclaredAccessibility == Accessibility.Public && m.Parameters.Length == 1).ToList();
            ...
            foreach (var f in candidateFactories)
            {
                ...
                // Try to infer expected index by conventional factory name
                int? expectedIndex = null;
                var name = f.Name;
                if (arity == 2 && name == "Ok") expectedIndex = 0;
                if (arity == 2 && name == "Error") expectedIndex = 1;
                if (name.StartsWith("Case") && name.Length > 4 && int.TryParse(name.Substring(4), out var parsed))
                {
                    expectedIndex = parsed;
                }

                if (expectedIndex.HasValue && expectedIndex.Value >= 0 && expectedIndex.Value < arity)
                {
                    ...
                    {
                        var diag = Diagnostic.Create(FactoryParameterMismatchDescriptor, f.Locations.FirstOrDefault() ?? typeSymbol.Locations.FirstOrDefault() ?? Location.None, f.Name, typeSymbol.Name, pType.ToDisplayString(SymbolDisplayFormat.MinimallyQualifiedFormat), expectedTypeParam.Name);
            ...
            var genAttr = typeSymbol.GetAttributes().FirstOrDefault(a => a.AttributeClass?.Name == "GenerateUnionAttribute" || a.AttributeClass?.Name == "GenerateUnion");
            ...
            for (int i = 0; i < arity; i++)
            {
                if (!factoryMap.ContainsKey(i))
                {
                    ...
                    var constructedTypeName = typeSymbol.Name + (arity > 0 ? ("<" + string.Join(",", Enumerable.Range(0, arity).Select(j => "T" + j)) + ">") : string.Empty);
                    var diag = Diagnostic.Create(MissingFactoryDescriptor, diagLocation, typeSymbol.Name, suggestedName, suggestedParam, constructedTypeName);
                    spc.ReportDiagnostic(diag);
                }
            }
            ...
            var constructedType = typeBase + "<" + string.Join(",", genericParams) + ">";
            ...
            sb.AppendLine("    /// This adapter is generated at compile-time and calls the generated union's static factory methods directly (no reflection).");
            sb.AppendLine("    /// It improves runtime performance and provides IntelliSense via XML documentation. Ensure the generated union exposes the expected static factories (e.g. Ok/Error).");
            sb.AppendLine("    /// </remarks>");
            ...
            sb.AppendLine($"        /// <exception cref=\"System.InvalidOperationException\">Thrown when the OneOf value cannot be mapped to a factory on the generated union type. See diagnostics UG2001/UG2002 for actionable fixes.</exception>");
            sb.AppendLine($"        /// <seealso cref=\"global::UnionGenerator.OneOfCompat.OneOfCompat\">Use OneOfCompat helpers if you prefer a reflection-based conversion.</seealso>");

            var genericDecl = "<" + string.Join(",", genericParams) + ">";
            sb.AppendLine($"        public static {typeBase}{genericDecl} FromOneOf{genericDecl}(this global::OneOf.OneOf<{string.Join(",", genericParams)}> one)");
            sb.AppendLine("        {");
            ...
            for (int i = 0; i < arity; i++)
            {
                ...
                {
                    sb.AppendLine($"                return {typeBase}{genericDecl}.{factoryName}(v);");
                }
                else
                {
                    // fallback: try common names by index (Case0/Case1) or default to throw with diagnostic
                    sb.AppendLine($"                throw new InvalidOperationException(\"No factory found for case {i} on type {typeSymbol.Name}\");");
                }
            ...
            sb.AppendLine();
            sb.AppendLine($"        /// <summary>Attempts to convert a OneOf<{string.Join(",", genericParams)}> to the generated union type without throwing.</summary>");
            for (int i = 0; i < genericParams.Length; i++)
            {
                sb.AppendLine($"        /// <typeparam name=\"{genericParams[i]}\">Case type {i}</typeparam>");
            }
            sb.AppendLine("        /// <param name=\"one\">The OneOf instance</param>");
            sb.AppendLine("        /// <param name=\"result\">When this method returns, contains the converted union if successful; otherwise default.</param>");
            sb.AppendLine("        /// <returns>True if conversion succeeded; otherwise false.</returns>");
            sb.AppendLine($"        public static bool TryFromOneOf{genericDecl}(this global::OneOf.OneOf<{string.Join(",", genericParams)}> one, out {typeBase}{genericDecl} result)");
            sb.AppendLine("        {");
            for (int i = 0; i < arity; i++)
            {
                var isProp = $"one.IsT{i}";
